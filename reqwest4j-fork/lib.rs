...
// Per-proxy client cache for fetchWithProxy: lets a single process route
// individual requests through different egress proxies (e.g. v4-direct vs a
// v6 proxy) at runtime, without re-init (which would panic on the OnceLock).
// Key "" = no proxy (direct egress). Clients are cheap to clone (Arc inside).
...
fn build_client(proxy: &str) -> Client {
    ...
    let builder = if proxy.is_empty() {
        builder
    } else {
        builder.proxy(reqwest::Proxy::all(proxy).unwrap())
    };
    builder.build().unwrap()
}

fn get_or_build_client(proxy: &str) -> Client {
    let map = CLIENTS.get_or_init(|| Mutex::new(HashMap::new()));
    let mut guard = map.lock().unwrap();
    if let Some(c) = guard.get(proxy) {
        return c.clone();
    }
    let client = build_client(proxy);
    guard.insert(proxy.to_string(), client.clone());
    client
}
...
pub extern "system" fn Java_rocks_kavin_reqwest4j_ReqwestUtils_fetch(
    ...
    run_fetch(env, url, method, body, headers, client)
}
...
pub extern "system" fn Java_rocks_kavin_reqwest4j_ReqwestUtils_fetchWithProxy(
    ...
    // Empty / unreadable proxy string => direct (no proxy) egress.
    let proxy_str = match env.get_string(&proxy) {
        Ok(s) => s.to_str().unwrap_or("").to_string(),
        Err(_) => String::new(),
    };
    let client = get_or_build_client(&proxy_str);
    run_fetch(env, url, method, body, headers, client)
}

// Shared request path for fetch + fetchWithProxy. Parses url/method/headers,
// builds the request from the supplied client, and resolves the returned
// CompletableFuture asynchronously. Identical wire behaviour to upstream's
// fetch; the only difference is where `client` comes from.
fn run_fetch(
    mut env: JNIEnv,
    url: JString,
    method: JString,
